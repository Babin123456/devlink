  ...
  return (
    <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ${className}`} role="status" aria-busy="true" aria-label="Loading content cards">
      {[...Array(count)].map((_, idx) => (
        <div
          key={idx}
          className="p-5 bg-slate-800/40 border border-slate-800 rounded-xl space-y-4 animate-pulse backdrop-blur-sm"
        >
          {hasAvatar && (
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-slate-700/60 shrink-0"></div>
          ...
          <div className="pt-2 flex items-center justify-between">
            ...
            <div className="h-6 bg-slate-700/40 rounded-md w-16"></div>
          </div>
        </div>
      ))}
    </div>
