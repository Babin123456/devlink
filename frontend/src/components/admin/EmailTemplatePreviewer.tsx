...
import { Mail, Code, Eye, FileText, Send, CheckCircle2, AlertCircle, RefreshCw } from 'lucide-react';
import {
  listEmailTemplates,
  renderEmailTemplate,
  EmailTemplateInfo,
...
export const EmailTemplatePreviewer: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 bg-slate-900/60 border border-slate-800 rounded-xl space-y-6 text-slate-100 backdrop-blur-md">
      {/* Top Header */}
        ...
        <div className="flex items-center bg-slate-800 p-1 rounded-lg border border-slate-700">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-md flex items-center gap-1.5 transition-all ${
              viewMode === 'html' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-md flex items-center gap-1.5 transition-all ${
              viewMode === 'text' ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            ...
            {templates.map((tpl) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {selectedType === tpl.template_type && <CheckCircle2 className="w-4 h-4 text-indigo-400" />}
                </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {currentInfo && renderData && (
            <div className="bg-slate-800/40 border border-slate-800 rounded-xl p-5 space-y-4">
              {/* Subject Line Header */}
              <div className="space-y-1 pb-3 border-b border-slate-700/60">
                <span className="text-xs text-slate-400 uppercase font-medium">Subject Line</span>
              ...
              {viewMode === 'html' ? (
                ...
                <div className="p-4 bg-slate-950 border border-slate-800 rounded-lg text-slate-200 font-mono text-xs whitespace-pre-wrap leading-relaxed max-h-[450px] overflow-y-auto">
