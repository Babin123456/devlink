...
def upgrade() -> None:
    ...
    op.add_column('audit_logs', sa.Column('metadata_info', postgresql.JSONB(astext_type=sa.Text()), nullable=True))

    # Add foreign keys
    op.create_foreign_key('fk_audit_logs_target_user_id', 'audit_logs', 'users', ['target_user_id'], ['id'], ondelete='SET NULL')
