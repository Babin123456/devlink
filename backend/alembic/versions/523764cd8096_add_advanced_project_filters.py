...
def upgrade() -> None:
    op.add_column('projects', sa.Column('language', sa.String(length=100), nullable=True))
    op.create_index(op.f('ix_projects_language'), 'projects', ['language'], unique=False)
    op.add_column('projects', sa.Column('experience', sa.String(length=50), nullable=True))
    op.create_index(op.f('ix_projects_experience'), 'projects', ['experience'], unique=False)
    op.add_column('projects', sa.Column('is_remote', sa.Boolean(), server_default='false', nullable=False))
    op.create_index(op.f('ix_projects_is_remote'), 'projects', ['is_remote'], unique=False)
    op.add_column('projects', sa.Column('is_paid', sa.Boolean(), server_default='false', nullable=False))
    op.create_index(op.f('ix_projects_is_paid'), 'projects', ['is_paid'], unique=False)
    op.add_column('projects', sa.Column('is_open_source', sa.Boolean(), server_default='false', nullable=False))
    op.create_index(op.f('ix_projects_is_open_source'), 'projects', ['is_open_source'], unique=False)
