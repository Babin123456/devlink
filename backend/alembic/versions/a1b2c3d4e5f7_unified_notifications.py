...
def upgrade() -> None:
    ...
    sa.Column('database_enabled', sa.Boolean(), nullable=False, server_default='true'),
    sa.Column('project_updates', sa.Boolean(), nullable=False, server_default='true'),
    ...
    notificationchannel = postgresql.ENUM('DATABASE', 'EMAIL', 'WEBSOCKET', name='notificationchannel')
    notificationchannel.create(op.get_bind())

    notificationstatus = postgresql.ENUM('PENDING', 'SENT', 'FAILED', 'READ', name='notificationstatus')
    notificationstatus.create(op.get_bind())

    notificationpriority = postgresql.ENUM('LOW', 'NORMAL', 'HIGH', 'URGENT', name='notificationpriority')
    notificationpriority.create(op.get_bind())

    op.add_column('notifications', sa.Column('channel', postgresql.ENUM('DATABASE', 'EMAIL', 'WEBSOCKET', name='notificationchannel', create_type=False), server_default='DATABASE', nullable=False))
    op.add_column('notifications', sa.Column('status', postgresql.ENUM('PENDING', 'SENT', 'FAILED', 'READ', name='notificationstatus', create_type=False), server_default='PENDING', nullable=False))
    op.add_column('notifications', sa.Column('priority', postgresql.ENUM('LOW', 'NORMAL', 'HIGH', 'URGENT', name='notificationpriority', create_type=False), server_default='NORMAL', nullable=False))
    
    op.add_column('notifications', sa.Column('metadata_info', postgresql.JSONB(astext_type=sa.Text()), nullable=True))
