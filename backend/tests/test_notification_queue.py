...
def test_notify_user_push(caplog):
    ...
    with patch("app.services.push_service.PushNotificationService.send_push") as mock_send:
        mock_send.return_value = True
        
        result = PushNotificationService.notify_user("user_123", "Hello", "World", action_url="http://test.com")
        
        assert result is True
        mock_send.assert_called_once_with(["device_token_for_user_123"], "Hello", "World", {"url": "http://test.com"})
...
def test_send_notification_task_success(mock_session_local, mock_push, mock_email, mock_notify, mock_get_user):
