...
def test_background_jobs_api_admin_only(client: TestClient):
    ...
    r = client.post(
        "/api/auth/login",
        json={"email": "user@test.com", "password": "Passw0rd!"}
    )
    token = r.json()["access_token"]
    
    r = client.get(
        "/api/admin/background-jobs/stats",
...
def test_background_jobs_api_success(client: TestClient, db: Session):
    ...
    r = client.post(
        "/api/auth/login",
        json={"email": "admin@test.com", "password": "Passw0rd!"}
    )
    token = r.json()["access_token"]
    ...
    job1 = BackgroundJob(
        ...
        payload={"args": [{"recipient_id": user_id, "sender_id": user_id, "type": "follow", "title": "a", "message": "b"}], "kwargs": {}},
