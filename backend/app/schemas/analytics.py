...
class DAUMetric(BaseModel):
    ...
    active_users: int = Field(..., description="Count of distinct active users on this day")
...
class RetentionMetric(BaseModel):
    ...
    retained_7d_users: int = Field(..., description="Count of retained users over 7 days")
    eligible_7d_users: int = Field(..., description="Count of users eligible for 7-day retention")
    retained_30d_users: int = Field(..., description="Count of retained users over 30 days")
    eligible_30d_users: int = Field(..., description="Count of users eligible for 30-day retention")
...
class ConversionMetric(BaseModel):
    ...
    completed_profiles_count: int = Field(..., description="Total completed user profiles")
    project_creators_count: int = Field(..., description="Total users with at least 1 project")
    total_applications_count: int = Field(..., description="Total builder flare applications")
    accepted_applications_count: int = Field(..., description="Total accepted applications")
...
class DailyProjectMetric(BaseModel):
    ...
    cumulative_projects: int = Field(..., description="Cumulative project count up to this date")
...
class ProjectGrowthMetric(BaseModel):
    ...
    new_projects_period: int = Field(..., description="New projects created in the specified period")
    growth_rate_pct: float = Field(..., description="Percentage growth in projects over the period")
...
class PlatformAnalyticsResponse(BaseModel):
    ...
    active_users: ActiveUsersOverview = Field(..., description="DAU, WAU, and MAU metrics")
    ...
    project_growth: ProjectGrowthMetric = Field(..., description="Project growth metrics and trend")
