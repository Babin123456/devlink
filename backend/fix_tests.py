...
def process_file(filepath):
    ...
    content = re.sub(
        r'engine = create_engine\(\s*"sqlite.*?poolclass=StaticPool,\n\)',
        "",
        content,
        flags=re.DOTALL,
    )
    content = re.sub(
        r"TestingSessionLocal = sessionmaker\(bind=engine, autoflush=False, autocommit=False\)\n",
        "",
        content,
    )

    content = re.sub(
        r"def override_get_db\(\):.*?finally:\n\s*db\.close\(\)\n",
        "",
        content,
        flags=re.DOTALL,
    )
